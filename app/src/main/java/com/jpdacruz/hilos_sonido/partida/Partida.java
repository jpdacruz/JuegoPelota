...
public class Partida extends androidx.appcompat.widget.AppCompatImageView {
    ...
    boolean pelota_sube;
    private Point maneja_coord;

    public Partida(Context contexto, int nivel_dificultad){

        super(contexto);

        setDisplaySize(contexto);

        setBackgroundSize(contexto);
    ...
    private void setDisplaySize(Context contexto) {
    ...
    private void setBackgroundSize(Context contexto) {
    ...
    private void setPelota(Context contexto) {
        ...
        posX=tam_pantX/2-tamPelota/2; //ubica la pelota en el medio de la pantalla y en el medio de la pelota

        posY=0-tamPelota; //ubica la pelota en la posicion 0 de y pero arriba, fuera de la pantalla
    }
    ...
    /**
    /metodo dibuja los drawbitmap en la pantalla
     */
